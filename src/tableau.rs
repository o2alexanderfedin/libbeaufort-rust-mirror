...
pub(crate) extern "C" fn beaufort_tableau(alpha: *const i8) -> *mut *mut i8 {
    ...
    mat =
        unsafe {
                calloc(size.wrapping_add(1 as u64),
                    core::mem::size_of::<*mut i8>() as u64)
            } as *mut *mut i8;
    if 0 as *mut () == mat as *mut () { return 0 as *mut () as *mut *mut i8; }
    {
        '__b9: loop {
            if !((y as u64) < size) { break '__b9; }
            '__c9: loop {
                unsafe {
                    *mat.offset(y as isize) =
                        unsafe { calloc(size, core::mem::size_of::<i8>() as u64) }
                            as *mut i8
                };
                if 0 as *mut () ==
                        unsafe { *mat.offset(y as isize) } as *mut () {
                    return 0 as *mut () as *mut *mut i8;
                }
                {
                    { x = 0; j = size as i32 };
                    '__b10: loop {
                        if !((x as u64) < size) { break '__b10; }
                        '__c10: loop {
                            unsafe {
                                *unsafe { (*mat.offset(y as isize)).offset(x as isize) } =
                                    unsafe { *alpha.add(((j + y) as u64 % size) as usize) } as
                                        i8
                            };
                            break '__c10;
                        }
